...
public:
		...
		virtual unsigned int GetAnalysisFlags();         // Flag from GPIO's (i-th)
		virtual void SetAnalysisFlags(unsigned int d);
		...
		// From pos    == Multi-Hit ==
		virtual double GetMHBkgPar(int tower, int layer, int xy,
		                           int ipar);             // Results of Background Fitting by a linear function
		virtual void SetMHBkgPar(int tower, int layer, int xy, int ipar, double val);
		virtual int GetMHNhits(int tower, int layer,
		                       int xy);                        // Number of Hits in the layer (layer, xy) includeing discarded peaks
		virtual void SetMHNhits(int tower, int layer, int xy, int nhits);
		virtual int GetMHParPos(int tower, int layer, int xy, int ihits,
		                        int ipar);   // Return the position of parameter in the array.
		virtual double GetMHPar(int tower, int layer, int xy, int ihit,
		                        int ipar);       // Parameter of Multi-hit fitting (layer, xy, ihit, ipar);
		virtual void
		SetMHPar(int tower, int layer, int xy, int ihit, int ipar, double val);  // To modify the parameter value.
		...
		virtual int GetMHNumberOfHits(
						int tower);                                    // Number of hits in the tower (except discarded peaks)
