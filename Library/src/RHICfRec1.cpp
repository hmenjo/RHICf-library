...
#define   REC_TYPE2_ENERGY(i, j)   (12+2*i+j)        // 12 + 4
#define   REC_TYPE2_POS(i, j, k)    (16+4*i+2*j+k)    // 16 + 8
#define  SIZE_CAL                 0
//#define   CAL_CAL(i,l)            (16*i+l)
//#define  SIZE_POS_INIT           136
#define   SIZE_POS_INIT           364
...
#define   POS_MHPOS(a, b, c, d)      (236+16*a+4*b+2*c+d)                  // a:tower, b:layer, c:xy, d:particle
...
unsigned int RHICfRec1::GetAnalysisFlags() { return flags[FLAGS_FLAGS(3)]; }

void RHICfRec1::SetAnalysisFlags(unsigned int d) { flags[FLAGS_FLAGS(3)] = d; }
// *** Newly developed for RHICf *** //

bool RHICfRec1::IsShowerTrig() {
	
	if ((FLAGS_FLAGS(0) & 0x010)) return true;
	else return false;
}

bool RHICfRec1::IsPi0Trig() {
	
	if ((FLAGS_FLAGS(0) & 0x080)) return true;
	else return false;
}

bool RHICfRec1::IsHighEMTrig() {
	if ((FLAGS_FLAGS(0) & 0x200)) return true;
	else return false;
}
...
double RHICfRec1::GetType2Energy(int it, int hit) { return rec[REC_TYPE2_ENERGY(it, hit)]; }

void RHICfRec1::SetType2Energy(int it, int hit, double d) { rec[REC_TYPE2_ENERGY(it, hit)] = d; }

double RHICfRec1::GetTyep2Position(int it, int ixy, int hit) { return rec[REC_TYPE2_POS(it, ixy, hit)]; }

void RHICfRec1::SetTyep2Position(int it, int ixy, int hit, double d) { rec[REC_TYPE2_POS(it, ixy, hit)] = d; }
...
void RHICfRec1::SetMHPeakEstimated(int it, int il, int ixy, int imu, double d) {
	pos[POS_MHYFUNC(it, il, ixy, imu)] = d;
}
