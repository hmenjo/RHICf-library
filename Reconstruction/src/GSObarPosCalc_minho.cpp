#ifndef __GSOBARPOSCALC_MINHO_CPP__
#define __GSOBARPOSCALC_MINHO_CPP__
...
#include "../include/GSObarPosCalc_minho.h"
...
void GSObarPosCalc::GetEachContribution(int tower){
	...
	//if(tower==1) cout << maxlay[tower] << endl;

	for(int il=0;il<4;il++){
		for(int ixy=0;ixy<2;ixy++){
			for(int imu=0;imu<2;imu++){

				//f_each[tower][il][ixy][imu] = (TF1*) get_Lorenzian(tower, 1);
...
double GSObarPosCalc::GetEachIntegral(int it, int il, int ixy, int imu){
	...
	if(it==1){ 
		//if(il==maxlay[it] && ixy==1) printf("imu=%d, Integral=%f\n", imu, f_each[it][il][ixy][imu] -> Integral(0, 40));
		return f_each[it][il][ixy][imu] -> Integral(0, 40);
	}
}
	...
	for(int it=0; it<GSObarPosCalc::NTOWER; ++it){
		for(int ilay=0; ilay<GSObarPosCalc::NLAYER; ++ilay){
			for(int ixy=0; ixy<GSObarPosCalc::NXY; ++ixy){
				...
				h_gsobar2[it][ilay][ixy]->Reset();//#UPDATED
				...
				for(int i=0; i<11; ++i){
					...
					f_multi_draw[it][ilay][ixy]->SetParameter(i,0);//#UPDATED
				}
	...
	for(int it=0; it<GSObarPosCalc::NTOWER; ++it){
		...
		for(int ilay=0; ilay<GSObarPosCalc::NLAYER; ++ilay){
			for(int ixy=0; ixy<GSObarPosCalc::NXY; ++ixy){
				...
				f_multi[it][ilay][ixy]->SetParLimits(1, x_prime[it][maxlay[it]][ixy]-2, x_prime[it][maxlay[it]][ixy]+2);//#UPDATED
				...
				f_multi[it][ilay][ixy]->SetParLimits(7, x_sub[it][maxlay[it]][ixy]-2, x_sub[it][maxlay[it]][ixy]+2);//#UPDATED
...
int GSObarPosCalc::InitialParamEstimate(){
	...
	double max_sumBar[2][4] = {0};// [tower][layer]
        ...
        for(int it=0; it<2; ++it){
                ...
                maxlay[it] = TMath::LocMax(4, max_sumBar[it]);//#UPDATED
                max_sumBar[it][maxlay[it]] = 0;
                maxlay2[it] = TMath::LocMax(4, max_sumBar[it]);//#UPDATED
        }
...
int GSObarPosCalc::EvalNumOfHits(int tower){
	...
	for(int il=0; il<4; ++il){
		...
		if(il==maxlay[tower] && nhit_each[tower][il][0]>0 && nhit_each[tower][il][1]>0) is_null=0;//#UPDATED
	}
	if( is_null == 1 ){
	...
	if(f_multi[tower][maxlay[tower]][0]->GetParameter(1)==f_multi[tower][maxlay[tower]][0]->GetParameter(7) && nhit_each[tower][maxlay[tower]][1]==2){//#UPDATED
	...
	if(f_multi[tower][maxlay[tower]][1]->GetParameter(1)==f_multi[tower][maxlay[tower]][1]->GetParameter(7) && nhit_each[tower][maxlay[tower]][0]==2){//#UPDATED
