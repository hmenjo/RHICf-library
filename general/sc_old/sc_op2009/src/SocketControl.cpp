...
int SocketControl::Connect(){
  ...
  if (hostinfo == NULL){
    cerr << "Unknown host " << serverhost << endl;
    return SOCKETCONTROL_ERROR;
  }
...
//   pthread_mutex_lock(&socketcontrol_mutex);
  ...
  if(send_command==true){ 
    ...
    nbytes = write (sock, command+i, numbytes+1-i);
    if (nbytes < 0) {
...
int SocketControl::GetBinaryData(LHCFSDATA* d){
  ...
  nbytes = write (sock, command,numbytes+1);
  if (nbytes < 0) {
    UnLock();
    return SOCKETCONTROL_ERROR;
  }
  ...
  while(1){
    ...
    tmpnbytes = recv(sock,&(answer[nbytes]),MAXDATA-nbytes,0);
    if (tmpnbytes < 0){
      UnLock();
      return SOCKETCONTROL_ERROR;
    }
