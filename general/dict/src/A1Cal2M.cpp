#ifndef __A1CAL2M_CPP__
#define __A1CAL2M_CPP__

#include "A1Cal2M.h"

//----------------------------------------------------------------------
// A1Cal2M  : Modified A1Cal2
//
//  This class has additional variables, TDC scaler and counter of GPIOs, 
//  which are same as "A1Cal1". 
//
// 
// +++ Logs +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//  20 Aug. 2015: First editted by H.MENJO
//----------------------------------------------------------------------
...
// -----                  A1Cal2M::clear                 -----       
// -----------------------------------------------------------
int A1Cal2M::clear(bankio option){
  // Clear data selectied. (default:ALL)
  // List of option 
  // 
  
  // cerr << "A1Cal2M::clear" << endl;
...
// -----------------------------------------------------------
// -----       A1Cal2M::copy copydata For A1Cal2         -----       
// -----------------------------------------------------------
int A1Cal2M::copy(A1Cal2* d, bankio option){ 
  // This is a virtual function for A1Cal2*. 
  // if d refers A1Cal2,  call A1Cal2::copy(A1Cal2* ,bankio);
  // if d refers A1Cal2M, call A1Cal2M::copy(A1Cal2M* ,bankio);
  if(strcmp(d->ClassName(),"A1Cal2")==0)  return A1Cal2::copy(d,option);
  if(strcmp(d->ClassName(),"A1Cal2M")==0) return A1Cal2M::copy((A1Cal2M*)d,option);
  return OK;
}

// -----------------------------------------------------------
// -----       A1Cal2M::copy copydata For A1Cal2M        -----       
// -----------------------------------------------------------
int A1Cal2M::copy(A1Cal2M* d, bankio option){ 
...
// -----------------------------------------------------------
// -----           A1Cal2M::copydata For A1Cal2          -----                      
// -----------------------------------------------------------
int A1Cal2M::copydata(A1Cal2* d, bankio option){ 
  // This is a virtual function for A1Cal2*. 
  // if d refers A1Cal2,  call A1Cal2::copydata(A1Cal2* ,bankio);
  // if d refers A1Cal2M, call A1Cal2M::copyata(A1Cal2M* ,bankio);
  if(strcmp(d->ClassName(),"A1Cal2")==0)  return A1Cal2::copydata(d,option);
  if(strcmp(d->ClassName(),"A1Cal2M")==0) return A1Cal2M::copydata((A1Cal2M*)d,option);
  return OK;
}

// -----------------------------------------------------------
// -----           A1Cal2M::copydata For A1Cal2M         -----                      
// -----------------------------------------------------------
int A1Cal2M::copydata(A1Cal2M* d, bankio option){
  // Copy only data of "d" to "this"
  // (default option:ALL)

  A1Cal2::copydata(d,option);
...
// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
int A1Cal2M::CheckBunchCrossing(){
  ...
  if(bptx1 > 0. && bptx2 > 0.){ return A1Cal2M::UNKNOWN;}
  
  // Check non-bunch crossing
  // Only Beam1 (coming from IP to Arm2 detector)
  if(bptx1 < 0. && bptx2 > 0.){ return A1Cal2M::NONCROSSING;}
  // Only Beam2 (out going from Arm2 detector to IP)
  if(bptx1 > 0. && bptx2 < 0.){ return A1Cal2M::NONCROSSING2;}  
  
  // Check displaced bunch crossing 
  if(fabs( bptx1-bptx2 ) > 50.){ return A1Cal2M::DISPLACED;}
  
  return A1Cal2M::CROSSING;
}

double A1Cal2M::GetTDCofBPTX1(){
