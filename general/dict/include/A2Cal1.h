...
// --------------------------------------------------------------------
//                               A2Cal1
//             For detail, see A2Cal1.cpp or Online Manual
// --------------------------------------------------------------------
...
class A2Cal1 : public TNamed {
 public:
  ...
  double         scl0[N_A2_SCL0];       // [scaler data]
 ...
 public:
  A2Cal1(){ clear() ;}
  A2Cal1(char* name, char *title) : TNamed(name,title){ clear();}
  ~A2Cal1(){;}
  
  int       clear(bankio option=ALL);
  int       copy(A2Cal1* d, bankio option=ALL);
  int       copydata(A2Cal1* d, bankio option=ALL);
  int       add(A2Cal1* d, bankio option=0x1FF);
  int       subtract(A2Cal1* d, bankio option=0x1FF);
  ...
  // ----- USEFUL FUNCTIONS -----
  int       CopySilicon(A2Cal2* d);
  double    GetTDCofBPTX1();
  double    GetTDCofBPTX2();
  int       CheckBunchCrossing();

  ClassDef(A2Cal1,2);   // Data structure for Arm2 data
};
