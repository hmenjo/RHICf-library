...
int main(int argc,char **argv){  
  ...
  RawL1T     *a2rawl1tlast = new RawL1T("a2rawl1tlast","Last Data of a2rawl1t");
  ...
  for(int itdc=0;itdc<=1;itdc++){
    a2hist_all->SetTDCHistogram(itdc, 500,-1100,-600);   
    a2hist_beam->SetTDCHistogram(itdc,500,-1100,-600); 
    a2hist_pede->SetTDCHistogram(itdc,500,-1100,-600);         
  }    

  for(int itdc=2;itdc<=6;itdc++){
    a2hist_all->SetTDCHistogram(itdc, 500,-500,500);   
    a2hist_beam->SetTDCHistogram(itdc,500,-500,500); 
    a2hist_pede->SetTDCHistogram(itdc,500,-500,500);         
  }    
  ...
  for(int i=0;i<nev;i++){
    ...
    if(ev->Check("a2rawl1t")){
      a2rawl1tlast->copydata((RawL1T*)ev->Get("a2rawl1t"));
    }
...
//     info->done_lumiscan = lumiscan;
  ...
  if(ofile_rundata){
    ...
    a2rawl1tlast->Write("a2rawl1tlast",TObject::kOverwrite);
