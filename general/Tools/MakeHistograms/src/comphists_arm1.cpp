...
int main(int argc,char **argv){ 
  ...
  for(int ic=0;ic<argc;ic++){ 
    ...
    if(strcmp(argv[ic],"-h")==0 || strcmp(argv[ic],"-help")==0 || argc==1){
      cout << endl
	   << " comphists_arm1 ------------------------------------------------------ \n"
	   << "  Draw some histograms made by adchists_arm1 in same canvas \n"
	   ...
	   << "\n"
	   << " options) \n"
	   << "  -i     :  Histogram file made by adchists_arm1.\n"
  ...
  double nev[10] = {0.};
  string sfilename[10];
  A1Cal1AccumHist* a1hist[10];

  for(int i=0;i<10;i++) ch[i] = false;

  char   dataname[16];
  if(dataid==0) strcpy(dataname,"a1hist_all");
  if(dataid==1) strcpy(dataname,"a1hist_beam");
  if(dataid==2) strcpy(dataname,"a1hist_pede"); 
  ...
  for(int i=0;i<nfile;i++){
    ...
    a1hist[i] = (A1Cal1AccumHist*) file[i]->Get(dataname);
    if(a1hist[i]==NULL){
    ...
    stmp = a1hist[i]->GetTitle();
    run[i] = atoi(stmp.substr(3,5).c_str());
    nev[i] = a1hist[i]->hist_cal[0][0][0]->GetEntries();
  ...
  A1Cal1AccumHist* canvas = new A1Cal1AccumHist();
  ...
  for(int i=0;i<nfile;i++){
    ...
    a1hist[i]->SetCanvasCal0(canvas->GetCanvasCal0());
    a1hist[i]->SetCanvasCal1(canvas->GetCanvasCal1());
    a1hist[i]->SetCanvasADCs(canvas->GetCanvasADCs());
    a1hist[i]->SetCanvasFlag(canvas->GetCanvasFlag());
    a1hist[i]->SetCanvasTDC(canvas->GetCanvasTDC());
    
    a1hist[i]->SetLineColor(color[i]);
    a1hist[i]->SetFillStyle(0);
    a1hist[i]->Scale(scale[i]);
    a1hist[i]->Draw(adcrange, checksame ? "same" : "");
    checksame = true;
  }
