...
int A1PedeSubtract::RHICfSubtract(A1Cal1* data, A1Cal1* pede){ 
  // ***** PEDESTAL DATA *****
  if(data->flag_pede){ 
    FillPedeEvent(data);
...
int A1PedeSubtract::RHICfSubtraction1(A1Cal1* data, int op){
  // for calorimeter
  for(int it=0;it<2;it++){
    for(int il=0;il<16;il++){
      for(int ir=0;ir<2;ir++){
...
// ******* A1PedeSubtract::ReadAveFile  **********************                                 
int A1PedeSubtract::ReadAveFile(char filename[]){
  ...
  while(!fin.eof()){
    if(fin.eof()) break;
    fin >> buf[0] >> buf[1] >> buf[2];
    if(j<16) calav[0][j] = buf[2];
    if(15<j && j< 32) calav[1][j-16] = buf[2];
    j++;
  }
  //fin.Close();
