...
int A1RawToCal1::Convert_RHICF_OP2017( RHICfRaw_Op2017* raw, A1Cal1* cal1){
	...
	/*for(int it=0;it<2;it++){
		for(int il=0;il<16;il++){
			printf("tower:%d, plate:%d, adcH: %1.0f, adcL: %1.0f\n", it, il, cal1->cal[it][il][0], cal1->cal[it][il][1]);	
			printf("tower:%d, plate:%d, adcHD: %1.0f\n", it, il, cal1->cald[it][il][0]);
		}
	}*/
	...
	//FILE* fTS;
	//fTS = fopen("/ccj/u/mhkim/RHICf/analysis/Reconstruction/STARdb/smallbarMap.txt", "w");
	//FILE* fTL;
	//fTL = fopen("/ccj/u/mhkim/RHICf/analysis/Reconstruction/STARdb/largebarMap.txt", "w");
		...
		for(int il=0;il<4;il++){
			for(int ixy=0;ixy<2;ixy++){
				...
				for(int ibar=0;ibar<nbar;ibar++){
					if(it==0) printf("tower:%d, layer:%d, xy:%d, bar:%d, adc = %1.0f\n", it, il, ixy, ibar, cal1->scifi0[il][ixy][ibar]);
					if(it==1) printf("tower:%d, layer:%d, xy:%d, bar:%d, adc = %1.0f\n", it, il, ixy, ibar, cal1->scifi1[il][ixy][ibar]);
				}
			}
		}
	}*/
