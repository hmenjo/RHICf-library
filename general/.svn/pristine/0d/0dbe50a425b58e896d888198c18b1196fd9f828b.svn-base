...
int main(int argc,char **argv){  
  ...
  for(int i=1;i<hbconf_a2_bptx1->GetNbinsX();i++){
    y = 0.9-0.022*total_nfill;
    if(hbconf_a2_bptx2->GetBinContent(i)>0.1 &&
       hbconf_a2_bptx1->GetBinContent(i)>0.1 ){
      sprintf(value,"%d",total_nfill++);
      latex->DrawTextNDC(x_t,y,value);
      sprintf(value,"-",arm1_nfill++);
    ...
    else if(hbconf_a2_bptx2->GetBinContent(i)>0.1){
       sprintf(value,"%d",total_nfill++);
      latex->DrawTextNDC(x_t,y,value);
      sprintf(value,"-",arm1_nfill++);
      latex->DrawTextNDC(x_a1,y,value);
