...
void makehist(string filename, array<TH1D,10> histarray, array<TH2D,10> maparray, array<TH3D,10> map3array)
{
    
    //Junsang****TFile* file = TFile::Open(filename.c_str());
    TFile* file = new TFile("tt.root", "RECREATE");
    file->Print();
    //Junsang****TFile* file = TFile::Open("/home/hoosiki/100GeV/N_TL_TL100GeV01033931.root");

    auto fARM1PL = make_shared<TTree>();
    auto fCanvas = make_shared<TCanvas>();
    auto fHist = make_shared<TH1D>();
    fHist->Fill(1);
    fCanvas->cd();
    //Junsang****fCanvas->Print("test.png", "png");
    //Junsang****TTree* fARM1PL;
    //Junsang****file->GetObject("ARM1PL", fARM1PL);
    //Junsang****cout << fARM1PL->GetEntries() << endl;
    
    cout << "this is from thread" << endl;
    cout << thread::hardware_concurrency() << endl;
    file->Close();
    //Junsang****delete file;

}
void makehist2(string filename, array<TH1D,10> histarray, array<TH2D,10> maparray, array<TH3D,10> map3array)
{
...
int main(int argc, char** argv)
{
    ...
    string tmpas;
    tmpas = "/home/hoosiki/100GeV/N_TL_TL100GeV01033931.root";
    array<TH1D,10> harray;
    array<TH2D,10> marray;
    array<TH3D,10> m3array;

    //Junsang****TFile* file = TFile::Open(tmpas);
    thread t(makehist, tmpas, harray, marray, m3array);
    thread t2(makehist, "/home/hoosiki/100GeV/N_TL_TL100GeV0101362.root", harray, marray, m3array);

    t.join();
    //Junsang****thread t2(makehist, tmpas, harray, marray, m3array);
    t2.join();
