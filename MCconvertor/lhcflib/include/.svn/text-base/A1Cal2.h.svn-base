...
class A1Cal2 : public TNamed {
 public:
  ...
  double         scifi0[4][2][20];   // Scifi 20mm tower [layer:xy:fiber]
  double         scifi1[4][2][40];   // Scifi 40mm tower [layer:xy:fiber]
 ...
 public:
  A1Cal2(){ clear() ;}
  A1Cal2(char* name, char *title) : TNamed(name,title){ clear();}
  ~A1Cal2(){;} 

  int       clear(bankio option=ALL);
  int       copy(A1Cal2* d, bankio option=ALL);
  int       copydata(A1Cal2* d, bankio option=ALL);
  int       add(A1Cal2* d, bankio option=0x6D);
  int       subtract(A1Cal2* d, bankio option=0x6D);
  int       multiply(double d, bankio option=0x6D);
  int       divide(double d, bankio option=0x6D);
  
  double    calsum(int it,int sl=0,int ie=15);   // Simple sum of calorimeter 
  double    calsum2(int it,int sl=0,int ie=15);  // Modified Sum (if layer>10,*2)
  double    scifisum(int it,int il,int ixy);     // Sum of Scifi dEs
  ...
  ClassDef(A1Cal2,1);  // Data structure for Arm1 data
};
